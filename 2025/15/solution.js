 ...
 * @returns {string}
 */
export function drawTable(data, sortBy) {
  ...
  const widths = Object.fromEntries(
    cols.map((col) => [col, Math.max(...data.map((row) => `${row[col]}`.length))])
  );

  const border =
    CORNER + cols.map((col) => ROW.repeat(widths[col] + 2)).join(CORNER) + CORNER;

  const getRow = (row) =>
    COL + cols.map((c) => ` ${`${row[c]}`.padEnd(widths[c])} `).join(COL) + COL + '\n';
